...
public class ProcessTransaction {
	 ...
	 *//*
		 * private void checkForMonthlyBalance(Map<String, Map<String, Double>>
		 * monthEndBalance, Transaction oneTransaction, Map<String, Double> monthlyMap,
		 * Map<String, Double> localMapForCalculation) {
		 * 
		 * try { LocalDateTime dateOfTransaction = oneTransaction.gettDateAndTime();
		 * String monthAndYearKey = dateOfTransaction.getMonth() + " " +
		 * dateOfTransaction.getYear(); Double amount = 0.0;
		 * 
		 * if(localMapForCalculation.containsKey(oneTransaction.gettSource())){ Double
		 * amt = localMapForCalculation.get(oneTransaction.gettSource()); amount+=amt; }
		 * 
		 * if(monthEndBalance.containsKey(oneTransaction.gettSource())) {
		 * monthlyMap=monthEndBalance.get(oneTransaction.gettSource()); //get month and
		 * year of the transaction
		 * 
		 * if(monthlyMap.containsKey(monthAndYearKey) &&
		 * oneTransaction.gettType().equals(TransactionType.CREDIT)) { amount =
		 * monthlyMap.get(monthAndYearKey); amount+=oneTransaction.gettAmount(); } else
		 * if(monthlyMap.containsKey(monthAndYearKey)) { amount =
		 * monthlyMap.get(monthAndYearKey); amount-=oneTransaction.gettAmount(); } else
		 * if(oneTransaction.gettType().equals(TransactionType.CREDIT))
		 * amount+=oneTransaction.gettAmount(); else
		 * amount-=oneTransaction.gettAmount();
		 * 
		 * monthEndBalance.get(oneTransaction.gettSource()).put(monthAndYearKey,
		 * amount); }else { if(oneTransaction.gettType().equals(TransactionType.CREDIT))
		 * { amount+=oneTransaction.gettAmount(); monthlyMap.put(monthAndYearKey,
		 * amount); monthEndBalance.put(oneTransaction.gettSource(), monthlyMap); } else
		 * { amount-=amount-oneTransaction.gettAmount(); monthlyMap.put(monthAndYearKey,
		 * amount); monthEndBalance.put(oneTransaction.gettSource(), monthlyMap); } }
		 * 
		 * if(!localMapForCalculation.containsKey(oneTransaction.gettSource())){
		 * localMapForCalculation.put(oneTransaction.gettSource(), amount); }else {
		 * localMapForCalculation.put(oneTransaction.gettSource(),
		 * localMapForCalculation.get(oneTransaction.gettSource())+amount); }
		 * 
		 * monthlyMap = new TreeMap<>(); localMapForCalculation = new TreeMap<>();
		 * }catch(Exception e) { logger.info("Exception in checkForMonthlyBalance: " +
		 * e); e.printStackTrace(); } }
		 */
	...
	private void takeInput() {
		try {
			transactionList.add(new Transaction("T001", Math.random() * 1000,
					LocalDateTime.of(2015, Month.JULY, 29, 19, 30, 34), "S1", "S2", TransactionType.DEBIT));
			transactionList.add(new Transaction("T002", Math.random() * 1000,
					LocalDateTime.of(2016, Month.JULY, 21, 11, 22, 23), "S1", "S2", TransactionType.DEBIT));
			transactionList.add(new Transaction("T003", Math.random() * 1000,
					LocalDateTime.of(2017, Month.AUGUST, 1, 10, 45, 12), "S1", "S2", TransactionType.CREDIT));
			transactionList.add(new Transaction("T004", Math.random() * 1000,
					LocalDateTime.of(2018, Month.AUGUST, 6, 16, 34, 45), "S1", "S2", TransactionType.DEBIT));
			transactionList.add(new Transaction("T005", Math.random() * 1000,
					LocalDateTime.of(2019, Month.JANUARY, 27, 21, 21, 12), "S1", "S2", TransactionType.CREDIT));
			/*
			 * transactionList.add(new Transaction("T006",Math.random()*1000,
			 * LocalDateTime.of(2017, Month.FEBRUARY, 14, 18, 12,
			 * 34),"S1","S2",TransactionType.DEBIT)); transactionList.add(new
			 * Transaction("T007",Math.random()*1000, LocalDateTime.of(2020,
			 * Month.SEPTEMBER, 17, 13, 12, 13),"S1","S4",TransactionType.CREDIT));
			 * transactionList.add(new Transaction("T008",Math.random()*1000,
			 * LocalDateTime.of(2020, Month.SEPTEMBER, 12, 13, 12,
			 * 13),"S1","S4",TransactionType.DEBIT)); transactionList.add(new
			 * Transaction("T009",Math.random()*1000, LocalDateTime.of(2020,
			 * Month.SEPTEMBER, 27, 13, 12, 13),"S1","S6",TransactionType.CREDIT));
			 * transactionList.add(new Transaction("T010",Math.random()*1000,
			 * LocalDateTime.of(2020, Month.SEPTEMBER, 22, 13, 12,
			 * 13),"S1","S2",TransactionType.DEBIT));
			 */

			transactionList.add(new Transaction("T011", Math.random() * 1000,
					LocalDateTime.of(2015, Month.JULY, 29, 19, 30, 34), "S2", "S1", TransactionType.DEBIT));
			transactionList.add(new Transaction("T012", Math.random() * 1000,
					LocalDateTime.of(2016, Month.AUGUST, 21, 11, 22, 23), "S2", "S3", TransactionType.CREDIT));
			transactionList.add(new Transaction("T013", Math.random() * 1000,
					LocalDateTime.of(2012, Month.JULY, 1, 10, 45, 12), "S2", "S1", TransactionType.DEBIT));
			transactionList.add(new Transaction("T014", Math.random() * 1000,
					LocalDateTime.of(2013, Month.AUGUST, 3, 23, 45, 12), "S2", "S1", TransactionType.CREDIT));
			transactionList.add(new Transaction("T015", Math.random() * 1000,
					LocalDateTime.of(2014, Month.MARCH, 1, 11, 45, 12), "S2", "S3", TransactionType.DEBIT));
			/*
			 * transactionList.add(new Transaction("T016",Math.random()*1000,
			 * LocalDateTime.of(2018, Month.MARCH, 6, 11, 45,
			 * 12),"S2","S3",TransactionType.CREDIT)); transactionList.add(new
			 * Transaction("T017",Math.random()*1000, LocalDateTime.of(2018, Month.OCTOBER,
			 * 16, 11, 45, 12),"S2","S3",TransactionType.DEBIT)); transactionList.add(new
			 * Transaction("T018",Math.random()*1000, LocalDateTime.of(2018, Month.JULY, 25,
			 * 11, 45, 12),"S2","S4",TransactionType.CREDIT)); transactionList.add(new
			 * Transaction("T019",Math.random()*1000, LocalDateTime.of(2018, Month.AUGUST,
			 * 24, 11, 45, 12),"S2","S6",TransactionType.DEBIT)); transactionList.add(new
			 * Transaction("T020",Math.random()*1000, LocalDateTime.of(2021, Month.JANUARY,
			 * 10, 11, 45, 12),"S2","S6",TransactionType.CREDIT));
			 */
